...
/* Bring-up test target: the isolated BACnet segment (10.0.3.0/16, no gateway).
   DHCP isn't available there, so use a static IP for this test. */
#define BRINGUP_STATIC_IP   "10.0.3.99"
#define BRINGUP_NETMASK     "255.255.0.0"
#define BRINGUP_GATEWAY     "0.0.0.0"
...
void app_main(void)
{
    ...
    if (eth_port_cnt == 1) {
        ...
        // No DHCP server on the target BACnet segment - use a static IP instead
        ESP_ERROR_CHECK(esp_netif_dhcpc_stop(eth_netifs[0]));
        esp_netif_ip_info_t ip_info = {0};
        ip4addr_aton(BRINGUP_STATIC_IP, (ip4_addr_t *)&ip_info.ip);
        ip4addr_aton(BRINGUP_NETMASK, (ip4_addr_t *)&ip_info.netmask);
        ip4addr_aton(BRINGUP_GATEWAY, (ip4_addr_t *)&ip_info.gw);
        ESP_ERROR_CHECK(esp_netif_set_ip_info(eth_netifs[0], &ip_info));
        ESP_LOGI(TAG, "Static IP configured: %s / %s / gw %s",
                 BRINGUP_STATIC_IP, BRINGUP_NETMASK, BRINGUP_GATEWAY);
    } else {
