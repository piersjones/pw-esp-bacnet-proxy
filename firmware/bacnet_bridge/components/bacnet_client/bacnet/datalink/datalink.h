...
/* h_bbmd.h intentionally not included here (ESP-IDF port): it expects
   upstream's full BACNET_IP_ADDRESS-based bip.h, but this component uses
   the simplified ESP32-port bip.h/bvlc.c pair instead (raw byte-array
   addressing, no BBMD support - not needed since this client talks
   directly to a known unicast target, no broadcast/foreign-device
   registration). h_bbmd.c/h_bbmd6.c are also not vendored in this
