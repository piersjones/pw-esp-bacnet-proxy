...
/**
 * @brief ESP-IDF port addition: tell bip_socket_esp_idf.c which esp_netif_t
 *        to report IP/netmask from (void* to avoid pulling esp_netif.h into
 *        this otherwise framework-agnostic header - cast to esp_netif_t* in
 *        the .c file that calls this).
 * @param netif esp_netif_t* for the interface BACnet/IP should bind to
 */
